# 👨‍💻 Jorge Chamorro – Ingeniería Informática PUCP

¡Bienvenido a mi repositorio! 👋  
Este espacio reúne mis proyectos, prácticas y documentación como estudiante de **Ingeniería Informática** en la **Pontificia Universidad Católica del Perú (PUCP)**. Aquí encontrarás desde sistemas académicos hasta experimentos de investigación, todos organizados con enfoque en **arquitectura modular**, **documentación profesional** y **buenas prácticas de desarrollo**.

---

## 🌎 Sobre mí
- 🎓 Estudiante de 8vo ciclo en Ingeniería Informática (PUCP).  
- 📚 Becario de **Beca 18 – Pronabec**, con desempeño académico constante y disciplina de trabajo.  
- 💡 Comprometido, entusiasta y con alto potencial de aprendizaje.  
- 🌐 Idiomas: Español (nativo) e Inglés (básico).  
- 🇵🇪 País: Perú

Me apasiona la tecnología y el desarrollo de soluciones informáticas. Me caracterizo por ser **organizado, eficiente y proactivo**, cualidades que aplico en proyectos académicos y personales.  

---

## 🚀 Tecnologías y Herramientas

### 🔹 Lenguajes de Programación
C, C++, C#, Java, Python, R, Go, Bash, JavaScript, Pseudocódigo  

### 🔹 Desarrollo Web
HTML, CSS, ASP.NET (ASPX), React  

### 🔹 Bases de Datos
MySQL, SQL Server, Oracle  

### 🔹 Control de Versiones
Git, Git Bash, GitHub  

### 🔹 Sistemas Operativos y Virtualización
Windows 10, Ubuntu Desktop, Linux Mint, VirtualBox  

### 🔹 Redes y Cloud
Packet Tracer, AWS Academy (instancias Windows/Ubuntu, VPN, seguridad, AD DS, Docker, Kubernetes)  

### 🔹 Herramientas de Desarrollo
Visual Studio Code, Visual Studio Community 2022, NetBeans, RStudio, JasperSoft Studio, Google Colab, PsychoPy, Figma, Drawio, Miro  

### 🔹 Gestión y Productividad
ClickUp, Jira, Freedcamp, Google Workspace, Microsoft Office Suite  

---

## 📖 Conocimientos Clave
- **Gobierno y Gestión de Servicios de TI (ITSM):** Ciclo de vida del servicio, gestión de incidentes, cambios, continuidad y activos de TI.  
- **Seguridad de la Información (ISO/IEC 27001:2022):** Gestión de riesgos, controles ISO/IEC 27002, continuidad del negocio (BCM), resiliencia, RPO/RTO.  
- **Ingeniería de Requisitos:** Casos de uso, historias de usuario, prototipado, validación de requisitos funcionales y no funcionales.  
- **Arquitectura y Documentación Técnica:** Notación C4, UML, patrones arquitectónicos y de diseño orientado a objetos.  
- **Pruebas de Software:** Principios de pruebas, TDD/BDD, pruebas de APIs (REST/SOAP).  
- **Inteligencia Artificial:** Machine Learning, Deep Learning, Lógica Difusa, métricas de evaluación y flujo de preprocesamiento de datos.  
- **Ética y Responsabilidad Social Universitaria:** Innovación responsable, sostenibilidad y compromiso ciudadano.  

---

## 🧑‍💻 Proyectos Destacados

- 🔗 [**SanchezLovers**](https://github.com/SanchezLovers)  
Repositorio grupal de programación realizado en equipo para un curso universitario.  

- 🔗 [**Prelearned Chunks Working Memory – PsychoPy**](https://github.com/darkJCdark/prelearned-chunks-working-memory-psychopy)  
Diseño experimental y análisis del estudio de Allen, DeStefano y Brady (2021) sobre cómo los fragmentos preaprendidos afectan la memoria de trabajo visual.  

- 🔗 [**Fraud Detection in Banking Transactions – ML**](https://github.com/darkJCdark/fraud-detection-bank-transactions-ml)  
Modelo de **Machine Learning** para la detección de transacciones bancarias fraudulentas.  

  **Contenido:**  
  - Documentación académica: Presentación y reporte con contexto, objetivos, metodología, resultados y conclusiones.  
  - Código fuente (Python/Scikit-learn): Preprocesamiento, análisis de correlaciones, escalamiento de variables, codificación de atributos categóricos y entrenamiento de algoritmos.  
  - Modelos evaluados: Regresión Logística y Random Forest con optimización de hiperparámetros.  
  - Resultados: El modelo **Random Forest** alcanzó un *recall* de **95%**, detectando casi todos los casos de fraude.  
  - Implicancias éticas: Privacidad de datos, sesgos, transparencia y seguridad frente a ataques adversarios.  

  **Objetivo:**  
  Desarrollar un sistema predictivo capaz de identificar patrones en transacciones bancarias y anticipar posibles fraudes, reduciendo falsos negativos y fortaleciendo la confianza en los sistemas financieros.  

  **Dataset:**  
  Conjunto de datos público (`fraud_data.csv`) con atributos temporales, geográficos, demográficos y financieros.  

  **Tecnologías utilizadas:**  
  - Python (Pandas, NumPy, Scikit-learn, Seaborn, Matplotlib)  
  - Algoritmos de clasificación supervisada  
  - GridSearchCV para optimización de hiperparámetros  

- 📌 Página personal: *En desarrollo, pronto disponible como repositorio.*  

---

## 🎯 Objetivos
- Consolidar buenas prácticas de desarrollo y documentación.  
- Mantener un portafolio profesional y organizado.  
- Facilitar la reproducibilidad y escalabilidad de proyectos.  
- Integrar innovación tecnológica con principios éticos y sostenibles.  

---

## 📫 Contacto
- 📧 **Email:** a20206234@pucp.edu.pe  
- 🔗 **LinkedIn:** [Jorge Chamorro Cayetano](https://www.linkedin.com/in/jorgechamorrocayetano/)  

---

> _“La ingeniería no solo resuelve problemas, también construye futuros.”_
